...
use rusty_tester::utils::data::{new_flood_request, new_flood_request_with_path};
...
#[allow(dead_code)]
fn assert_n_drone_event(options: &Node, n: usize, drone_event: &DroneEvent) {
    for _ in 0..n {
        options.assert_expect_drone_event(drone_event);
    }
}
...
fn test_drone_flood_req_inclusive() {
    ...
    assert_n_drone_event(&options, 2, &PacketSent(expected));
...
fn test_drone_flood_res_inclusive() {
    ...
    // Require extra node to be sure that it is not a leaf
    let (options, mut drone, packet_exit, _) = simple_drone_with_two_exit(11, 1.0, 10, 13);
    drone.handle_packet(&packet, false);
    drone.handle_packet(&packet, false);
    ...
    #[cfg(feature = "packet_sent_for_flood")]
    assert_n_drone_event(
        &options,
        1,
        &PacketSent(new_flood_request_with_path(
            5,
            7,
            10,
            &[(10, NodeType::Client), (11, NodeType::Drone)],
        )),
    );
    options.assert_expect_drone_event(&PacketSent(expected));
    options.assert_expect_drone_event_fail();
}
