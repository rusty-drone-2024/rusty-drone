...
use crate::testing_utils::{test_initialization_with_value, test_muliple_initialization, DroneOptions};

const TIMEOUT_RECV : Duration = Duration::from_millis(100);

#[test]
fn test_drone_packet_fragment(){
    let mut drones: Vec<(DroneOptions, MyDrone)> = test_muliple_initialization(3);
    let (_options1, mut drone1) = drones.pop().unwrap();
    let (options2, mut drone2) = drones.pop().unwrap();
    let (options3, drone3) = drones.pop().unwrap();

    assert!(!drone1.handle_commands(DroneCommand::AddSender(drone2.id, options2.packet_drone_in.clone())));
    assert!(!drone2.handle_commands(DroneCommand::AddSender(drone3.id, options3.packet_drone_in.clone())));
    ...
    drone2.handle_packet(
        packet.clone(),
        false,
    );
    let drone2_event = options2.event_recv.recv_timeout(TIMEOUT_RECV).unwrap();
    match drone2_event {
        DroneEvent::PacketSent(sent_packet) => assert_eq!(packet.session_id, sent_packet.session_id),
        _ => assert!(false)
    }
}
...
fn test_drone_packet_forward(){
    ...
    let forwarded_packet = new_receiver.recv_timeout(TIMEOUT_RECV).expect("Timeout on recv");
    assert_eq!(packet.clone(), forwarded_packet);

    let event = options.event_recv.recv_timeout(TIMEOUT_RECV).expect("Timeout on recv");
